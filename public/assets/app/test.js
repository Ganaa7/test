...
function editEstudiante(id)
{
    ...
    $.ajax({
        url : base_url+"/test/ajax_edit/"+id,
        type: "GET",
        dataType: "JSON",
...
function save()
{
    ...
    if(save_method == 'add') {
        url = base_url+"/test/ajax_add";
    } else {
        url = base_url+"/test/ajax_update";
    }
    ...
    $.ajax({
        ...
        error: function (jqXHR, textStatus, errorThrown)
        {
            ...
            $('#btnSave').text('Хадгал'); //change button text
            $('#btnSave').attr('disabled',false); //set button enable
...
function deleteEstudiante(id)
{
    if(confirm('Та энэ мэдээллийг устгахдаа итгэлтэй байна уу?'))
    {
        ...
        $.ajax({
            url : base_url+"/test/ajax_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                type: "POST",
                data: {id:list_id},
                url: base_url+"/test/ajax_list_delete",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
